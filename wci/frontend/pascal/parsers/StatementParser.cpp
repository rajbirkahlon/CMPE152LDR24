    ...
    throw (string)
{
           ...
           && (token->get_type() != (TokenType) terminator))
    {
    	...
    	{
    		//this is an error, WHEN can only exist in LOOP-AGAIN statement
    		error_handler.flag(token, UNEXPECTED_WHEN, this);
    		token = next_token(token);//consume WHEN
    		terminator_set.erase(PT_IDENTIFIER);
    		token = synchronize_after_when(terminator_set);
    		terminator_set.insert(PT_IDENTIFIER);
    	}
    	else
    	{
...
void StatementParser::parse_loop_again_list(Token *token, ICodeNode *parent_node,
    ...
    throw (string)
{
    // Synchronization set for the terminator.
    EnumSet<PascalTokenType> terminator_set(STMT_START_SET);
    terminator_set.insert(terminator1);
    terminator_set.insert(terminator2);


    // Loop to parse each statement until the END token
    // or the end of the source file.
    while (   (token != nullptr)
           && ((token->get_type() != (TokenType) terminator1)
        		   && (token->get_type() != (TokenType) terminator2)))
    {
        // Parse a statement.  The parent node adopts the statement node.
        ICodeNode *statement_node = parse_statement(token);
        parent_node->add_child(statement_node);
    ...
    // Look for the terminator token.
    if ((token->get_type() == (TokenType) terminator1) || (token->get_type() == (TokenType) terminator2))
    {
        //in loop-again case, we do not want to consume the terminator token for the 2nd WHEN
    	//the terminator token is how we determine if the loop continues
    }
    else {
        error_handler.flag(token, error_code, this);
    }
